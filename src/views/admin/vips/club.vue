  ...
  <div>
    <div class="crumb-box">
      <el-breadcrumb separator-class="el-icon-arrow-right" class="bread-crumb">
        ...
        <el-breadcrumb-item>俱乐部会员</el-breadcrumb-item>
      </el-breadcrumb>
      <el-input
          ...
          <template slot-scope="scope">
            <el-col :offset="3" :span="17">
              ...
              <div class="prohibtit-box" v-show="scope.row.darkroom != 0">
                <p>小黑屋:</p>
                <p>{{ getBlackListTime(scope.row.darkroom) }}</p>
                <my-button
      ...
      ><div style="display:flex">
        ...
        <el-form v-model="actyDetail">
          <el-form-item label="活动名称："
            ><span>{{ actyDetail.name }}</span></el-form-item
          >
          <el-form-item label="举办地点:"
            ><span>{{
              actyDetail.location + "-" + actyDetail.detailedLocation
            }}</span></el-form-item
          >
          <el-form-item label="奖金："
            ><span>{{ "￥" + actyDetail.reward }}</span></el-form-item
          >
          <el-form-item label="报名费用："
            ><span>{{ "￥" + actyDetail.cost }}</span></el-form-item
          >
...
export default {
  ...
  methods: {
    ...
    async getClubUsers() {
      ...
      this.tableData = respData.clubBeans;
    },
    // 通过uuid获取头像
    getAvatarUrl(data) {
      // 数据库中没有头像的数据，则使用默认头像
